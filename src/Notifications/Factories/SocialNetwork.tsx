...
// @ts-ignore
const SocialNetworkNotificationFactory: React.FC<IProps> = ({
  notificationItem,
}) => {
  ...
  const acceptRequestFriend = (e: any) => {
    ...
    api
      .put(
        `${links.api.social}friends/${notificationItem.common.connection_id}`
      )
      .then((response: any) => {
        if (response.status === 204) {
          api
            .put(`${links.api.notifications}${notificationItem._id}/accept`, {
              accept: true,
            })
            .then((response: any) => {
  ...
  const excludeRequestFriend = (e: any) => {
    ...
    api
      .delete(
        `${links.api.social}connections/${notificationItem.common.connection_id}`
      )
      .then((response: any) => {
        if (response.status === 204) {
          api
          ...
          >
            <NotificationContentText>
              <label>
                ...
                te enviou uma solicitação de amizade.
              </label>
            </NotificationContentText>
            <Stack direction="row" spacing={1} style={{ marginTop: 0 }}>
              ...
              {notification.common.status === 'accepted' && (
                <small>Pedido aceito</small>
              )}
              {notification.common.status === 'refused' && (
                <small>Pedido recusado</small>
              )}
            </Stack>
          </NotificationContainer>
        );
            ...
            <NotificationContentText>
              <label>
                ...
                {notificationItem.type ===
                  notificationType.PUBLICATION_TYPE_LIKE && ' gostou da '}
                {notificationItem.type ===
                  notificationType.PUBLICATION_TYPE_COMMENT && ' comentou na '}
                {notificationItem.type ===
                  notificationType.PUBLICATION_TYPE_SHARE && ' compartilhou '}
                sua publicação
                {notificationItem.common.content && (
                  <>
                    : "
                    <NotificationHighlight>
                      {reduceString(notificationItem.common.content, 60)}
                    </NotificationHighlight>
                    "
                  </>
                )}
        ...
        return (
          <NotificationContainer
            url={`${links.web.social}p/${
              notificationItem.type ===
              notificationType.NEW_RECOMMENDATION_RECEIVED
                ? profile?.username
          ...
          >
            <NotificationContentText>
              <label>
                ...
                {notificationItem.type ===
                  notificationType.NEW_RECOMMENDATION_REQUEST &&
                ...
                {notificationItem.type ===
                  ...
                  <>
                    ...
                    <NotificationHighlight>
                      "{reduceString(notification.common.content, 55)}"
                    </NotificationHighlight>
                  </>
                )}
              </label>
        ...
        return (
          ...
          >
            <NotificationContentText>
              ...
              {notificationItem.common.publication_owner_id ===
              profile?.profile_id ? (
                ' na sua publicação '
              ) : (
                <>
                  na publicação de{' '}
                  <NotificationHighlight>
                    {notificationItem.common.publication_owner_name},{' '}
                  </NotificationHighlight>
                </>
              )}
              <NotificationHighlight>
                "{reduceString(notificationItem.common.content, 35)}"
              </NotificationHighlight>
            </NotificationContentText>
          </NotificationContainer>
        );
